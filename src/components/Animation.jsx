...
const Animation = () => {
  ...
  const [openPreviousAccordionIndex, setOpenPreviousAccordionIndex] =
    useState(0);
  ...
  useEffect(() => {
    ...
    if (openAccordionIndex !== null) {
      ...
      imageRefs.current.forEach((img, idx) => {
        if (
          img &&
          idx !== openAccordionIndex &&
          idx !== openPreviousAccordionIndex
        ) {
    ...
    parentRefs.current.forEach((parent, index) => {
      ...
      if (openPreviousAccordionIndex != null) {
        gsap.set(contentRefs.current[openPreviousAccordionIndex], {
          opacity: 0,
        });
        gsap.set(buttonRefs.current[openPreviousAccordionIndex], {
          opacity: 0,
        });
      }
  ...
  return (
    <div className="flex items-center justify-center py-8 sm:py-10 md:py-14 lg:py-20 px-4 sm:px-6 md:px-10 lg:px-0">
      <div className="flex flex-col md:flex-row rounded-2xl sm:rounded-3xl md:rounded-[40px] overflow-hidden max-w-355 w-full pt-8 sm:pt-10 pb-0 md:pl-4 lg:pl-5 md:pr-0 md:py-0 gap-5 lg:gap-0 md:gap-0  h-125 sm:h-145 md:h-180 lg:h-187.5 xl:h-187 relative gradient-bg box-6">
        {/* Desktop Navigation */}
        <div className="hidden md:flex absolute left-4 lg:left-6 top-1/2 -translate-y-1/2 flex-col space-y-3 z-30">
          <button
            ...
            className={`bg-white/40 backdrop-blur-sm w-8 h-8 rounded-full flex justify-center items-center transition-all duration-500 ${
              openAccordionIndex === 0
                ...
                : "cursor-pointer hover:bg-white/90 hover:scale-110 active:scale-95"
            }`}
            disabled={openAccordionIndex === 0}
            onClick={() => changeAccordionByButton("previous")}
          >
            <FaChevronUp aria-hidden="true" className="text-slate-900" />
          </button>

          <button
            aria-label="Next section"
            className={`bg-white/40 backdrop-blur-sm w-8 h-8 rounded-full flex justify-center items-center transition-all duration-500 ${
              openAccordionIndex === accordionData.length - 1
                ? "bg-gray-400 cursor-not-allowed"
                : "cursor-pointer hover:bg-white/90 hover:scale-110 active:scale-95"
            }`}
          ...
          >
            <FaChevronDown aria-hidden="true" className="text-slate-900" />
          </button>
        </div>
              ...
              {accordionData.map((item, index) => (
                <div key={index} className="relative text-black w-full">
                  ...
                  >
                    ...
                    >
                      <span className="border border-gray-900 p-1 rounded-full">
                        <FaPlus />
                      </span>
                      <span className="text-lg lg:text-xl font-bold text-[#3B3A3A] whitespace-nowrap">
                        {item.title}
                      </span>
                    </h4>
                    ...
                    >
                      <h5 className="md:text-base lg:text-lg xl:text-xl font-bold md:mb-1 xl:mb-4 text-[#3B3A3A]">
                        {item.heading}
                      </h5>
          ...
          <div className="md:hidden">
            <div className="bg-white/30 backdrop-blur-md p-3 sm:p-4 rounded-xl sm:rounded-2xl mx-4 sm:mx-6">
              <h4 className="font-semibold mb-2 text-[#3B3A3A] text-base sm:text-lg leading-[139.1%]">
                {accordionData[openAccordionIndex].title}
              </h4>
        ...
        <button
          aria-label="Previous section"
          className="md:hidden absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-white/40 rounded-full z-10"
          onClick={() => changeAccordionByButton("previous")}
        >
          <svg
            aria-hidden="true"
        ...
        <button
          aria-label="Next section"
          className="md:hidden absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white/40 rounded-full z-10"
          onClick={() => changeAccordionByButton("next")}
        >
          <svg
            aria-hidden="true"
